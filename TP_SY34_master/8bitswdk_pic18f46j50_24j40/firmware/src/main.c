...
void RX(void){
    if(MiApp_MessageAvailable()){
        ...
        //uartPrint(rxMessage.SourceAddress);
...
void UniCast_Tx(void){
    ...
    static int Rb2_button = 1;

    char buffer_msg[30];
    sprintf(buffer_msg, "UNICAST Nom : %s numero: %d", myPseudo, counter);

    int i = 0;

    if (PORTBbits.RB0 != Rb0_button) {       
        Rb0_button = PORTBbits.RB0;
        if (PORTBbits.RB0 == 0) {
            uartPrint(buffer_msg);
            uartPrint("\r\n");

            while (buffer_msg[i] != 0) {
                MiApp_WriteData(buffer_msg[i]);
                i++;
            }
            counter++;
            MiApp_WriteData(10);
            ...
            if (myShortAddress.Val == 0x0000) {
                ...
                adr0 = myShortAddress.Val;
                uartPrint("probleme d'adresse");
            }
            MiApp_UnicastAddress((uint8_t * )&adr0, false,false);    
        }
    }
    if (PORTBbits.RB2 != Rb2_button) {
        
        Rb2_button = PORTBbits.RB2;
        if (PORTBbits.RB2 == 0) {
            uartPrint(buffer_msg);
            uartPrint("\r\n");
